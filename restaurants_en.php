     ...
     <h1>Restaurants</h1>
      <div class="filtre">
        <form>
          ...
          <hr>
          <br>
          <label for="stars-filter">Filter by star :</label>
          <div id="stars-filter">
      ...
      <div class="cards">
        ...
        try {
          $resultat = $cnx->prepare("SELECT * FROM restosen");
          $resultat->execute(); // Exécuter la requête pour récupérer les résultats

          while ($ligne = $resultat->fetch(PDO::FETCH_ASSOC)) {
            $hotelName = $ligne['nomresto'];
            $hotelAddress = $ligne['adrresto'];
            $imageUrl = $ligne['img'];
            $stars = $ligne['etoiles'];
            $priceLevel = $ligne['prix'];
            $tags = $ligne['tags'];
            $lien = $ligne['linkresto'];
            ...
            <div class="card-restaurant-hotel">
              ...
              <?php
                echo "<a class='card-website' href='$lien'>Restaurant website</a>";
              ?>
            </div>
          <?php
          }
